...
def calculate_fid_given_image_lists(
    image_lists, batch_size, device, dims, num_workers=1
):
    """Calculates the FID of two image list"""
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]

    model = InceptionV3([block_idx]).to(device)

    m1, s1 = compute_statistics_of_path(
        image_lists[0], model, batch_size, dims, device, num_workers
    )
    m2, s2 = compute_statistics_of_path(
        image_lists[1], model, batch_size, dims, device, num_workers
    )
    fid_value = calculate_frechet_distance(m1, s1, m2, s2)
...
if __name__ == "__main__":
    ...
    # expand image lists to equal with dims
    # image1_list = image1_list + image1_list[:dims - len(image1_list)]
    # image2_list = image2_list + image2_list[:dims - len(image2_list)]

    print("Number of images in image1_list: ", len(image1_list))
    print("Number of images in image2_list: ", len(image2_list))

    fid_value = calculate_fid_given_image_lists(
        [image1_list, image2_list], batch_size, device, dims, num_workers
    )
